...
| 监控 | TPS / 内存双曲线图（默认每 1 秒自动刷新，可在 `modules.panel.monitor-refresh-seconds` 调整）、最近性能事件、启动关闭会话记录（需开启 monitor 模块） |
| 文件 | 浏览 / 预览 / 编辑 / 删除 `data` 与 `userdata` 下的文件，单文件下载与整目录打包（需开启 `modules.panel.file-manager`） |
...
开启后面板多出「文件」页签，可以浏览 `data`（经济流水、监控采样等 JSONL）
与 `userdata`（每玩家一份的档案数据）：

- **预览**：文本文件按页加载（每页 500 行），超长行截断标注；非文本文件只显示大小、仅可下载
- **编辑**：≤ 2 MB 的文本文件可在线编辑，保存前按扩展名校验格式（YAML / JSON 严格校验，
  JSONL 逐行校验），格式错误会拒绝保存；注释与缩进原样保留
- **删除**：二次确认，**删除前自动备份**到 `plugins/EssentialEngine/backups/<时间戳>-<root>/`
- **下载**：单个文件，或把整个 `data` / `userdata` 打包成 zip（zip 条目标签带目录前缀，方便解压后识别）

安全边界：路径严格锁定在两个根目录内，`../`、绝对路径与符号链接逃逸一律拒绝；
所有修改 / 删除 / 下载操作写审计日志（含来源 IP）；接口只挂在面板内部，
**不会**暴露给 webapi / AstrBot。注意 `transactions.jsonl` 这类正在被插件写入的文件，
在线编辑后可能被插件下一次落盘覆盖，修改前建议先备份。
