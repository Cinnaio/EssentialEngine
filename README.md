...
| 网页管理面板 | `panel` | 无命令，浏览器里改配置 / 管玩家（默认关闭） |
...
在浏览器里查看服务器状态、直接编辑 `config.yml`、管理在线玩家。
...
| 玩家 | 在线玩家列表与踢出 / 封禁 / 禁言 / 改余额 |
| 控制台 | 以控制台身份执行命令（**默认关闭**，见下） |
...
- **控制台默认关闭**（`allow-console: false`）。确实需要时再打开，
  并尽量用 `allowed-commands` 限定可执行的命令。
